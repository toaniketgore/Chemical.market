...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // When the page loads, check which filter is active
  const activeButton = document.querySelector(".filters button.filter-active");
  if (activeButton) {
    // Get the filter value of the active button
    const filterValue = activeButton.getAttribute("data-filter");

    // Filter items based on the active button's filter
    filterGridItems(filterValue);
  } else {
    // If no filter is active, show all by default
    filterGridItems("*");
  }
});
